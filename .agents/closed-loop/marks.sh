...
# なぜ打刻で、なぜ記録だけでは足りないのかは [0161](../../docs/adr/0161-development-window-as-feedback-unit.md)
# 「打刻が第一で、記録は補完」。
#
# 単位はセッションではなく**窓**なので、打刻は checkout ごとではなく窓ごとに綴じる。何が窓を開き
# 何が閉じるかは下の ROTATING_SOURCES が持つ。なぜセッションが単位として壊れているかは同じ ADR。
#
# 打刻はすべて**イベントの列**である。1 つの名前に 1 ファイル、1 行 1 epoch、常に追記。読む側は
# 最初の行・最後の行・行数のうち、問いが要るものを取る ——「実装はいつ始まったか」と「何回
# レビューしたか」が同じ記録から出る。どの打刻が繰り返しうるかを、前もって決めなくてよくなる。
#
...
#
# 打刻は checkout の追跡外 `tmp/` に住む（[0160](../../docs/adr/0160-agent-environment-loop.md) 決定 4）。
#
