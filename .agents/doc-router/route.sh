#!/usr/bin/env sh
# 編集しようとしているパスを統べている文書を名指す。
#
# 2 つのモードを持つ。引数にパスを渡すと 1 行 1 件で出す。`--hook` を渡すと PreToolUse の
# ペイロードを標準入力から読み、additionalContext を返す。
#
# **答えるのは「どの文書がこれを統べているか」だけで、何をすべきかは言わない。**書こうとした
# 瞬間に行き先を名指すことに価値があり、そこから先は読み手の仕事である。対応表そのものは
# routes.conf が持つ。
#
# **エントリの無いパスでは何も出さない。**表が不完全であることは欠陥ではなく、黙って通ることで
# いつもの索引読解に落ちる。何か言おうとすると、当たらなかったことを報告する行だけが増える。
...
# 制御文字を落として 1 行へ均す。対応表の値はリポジトリの内容であって指示ではないので、
# 改行を含む値が封筒の中で別の段落として読まれないようにする。
sanitize() {
