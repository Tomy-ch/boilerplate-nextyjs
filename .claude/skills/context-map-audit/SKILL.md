...
description: >-
  Read-only audit of `docs/design/context-map.md` against the code, reporting three divergences and editing
  nothing — a contact point in the tree with no edge on the map, an edge whose counterpart no longer exists,
  and an edge whose recorded translation no longer matches what the dependency matrix enforces. Use it before
  a release, after work that added or removed an outward-facing entry point, or as a periodic sweep —
  「地図と実物が合ってる？」「この口は載ってる？」. It does not edit, because a divergence reads two ways — the map may be stale, or
  the code may have drifted from a decision — and choosing is the reader's. Every run states how many edges it
  checked and how many it could not. Do NOT use it to update the map (`context-map`) or to judge whether an
  edge should exist.
