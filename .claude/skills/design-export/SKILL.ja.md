...
script ではなくスキルとして存在する理由は、ADR [0055](../../../docs/adr/0055-design-system-export.md) が引いている線にある。**成果物はツール非依存で、運び方はそうではない。** `scripts/design-bundle` はどの SaaS も知らないし、その状態を保たなければならない。特定の vendor 名をリポジトリの恒久文書へ持ち込むことは ADR [0010](../../../docs/adr/0010-standards-and-non-lockin.md)（標準準拠と非ロックイン）に反する。したがって vendor の形をしたものはすべてここに置く。ここだけを差し替えれば、リポジトリに触れずに送り先を替えられる。
...
ADR [0055](../../../docs/adr/0055-design-system-export.md) が依存の向きを repo → ツールに固定しており、このスキルはそれを曲げてはならない。**デザインツールが生成したものを、このスキルがリポジトリへ書き込むことはない。** トークンも、component のソースも、スクリーンショットもである。デザイン提案を実装したい場合は、人間が何を採るかを決める、別の通常の実装作業になる。

理由は、デザインツールの出力が *どう見えるべきかについての提案* であるのに対し、リポジトリは *実際に出荷されるもの* だからである。提案が自動で流れ込めば、コードは誰も検証していないツールを追い始め、そのツールはテンプレートから作ったすべてのリポジトリが引き継ぐ上流になる。
