...
name: verify-spec
description: >-
  Reconcile the screen specifications under `docs/spec/route/**` with the implementation they describe, by reading them — the half of the reconciliation a machine cannot reach. Confirms scope once, resolves each route's spec directory through the same mapping the gate uses, then fans out the read-only `spec-validator` agent IN PARALLEL, one invocation per route, each reading that route's screen and functional requirements together with the code they map to. Four findings come back: a promise the implementation does not keep, a statement filed in the wrong one of the two documents, a screen restating what a layout above it already promises, and content the spec deliberately does not carry. It never re-derives whether a route has a spec or whether a spec outlived its route — a gate settles that, and doing it here would be the same check with worse tools. It never decides which side moves either: whether the promise changed or the implementation drifted is not visible from the diff, so both readings are reported and the human chooses. Use it before a release, after a screen's behaviour changed, or when a spec and the screen seem to disagree — 「仕様書と実装が合ってる？」「この約束まだ守られてる？」. Read-only on both sides; it writes nothing at all. Do NOT use it to check that specs exist (the gate does), to review a diff's implementation quality (`impl-review`), or to write a specification (`new-feature`).
