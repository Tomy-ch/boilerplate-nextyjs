...
本 ADR は、個別の技術決定に先立つ **設計判断の軸(判断軸)** のうち、**恒久的なもの** を定める。すなわち「**何を取り込むか(標準への準拠)/ その決定をどう正当化するか(非ロックインの判定)/ その判断を誰に向けて下すか(製品はテンプレートから作ったリポジトリが受け取る状態)**」という、全 ADR に横断して働くメタ原則である。
...
- **設計思想・プロバイダ・実装詳細は作った側の嗜好へ委ねる**。boilerplate が固定するのは seam の形であって、その中身の選択ではない。
- 例(認証 seam・[0079](0079-auth-frontend-seam.md)): Next.js 公式 auth ガイド(`node_modules/next/dist/docs/01-app/02-guides/authentication.md`)の文書化パターン(httpOnly session cookie / optimistic proxy + DAL による確定認可 / DTO)に乗る。乗らない=中立発明は「車輪の再発明」であり、本 boilerplate の哲学「考えないでもフロントが組める」([0011](0011-no-docker.md))を裏切る。
...
### 3. 製品はテンプレートから作ったリポジトリが受け取る状態である

本リポジトリの製品は、**テンプレートから作られた時点で受け取る状態**であって、それを作った履歴ではない。選択肢を比べ、推奨を述べるときは、この snapshot の上で比べる —— このリポジトリを見たことがなく、その git log も読まない読み手に、一貫したものとして読めるか。
...
- **姿勢(posture)そのものが製品である。** ブランチ保護・依存の固定・token 権限・security policy といったリポジトリ設定は、宣言ファイルとセットアップ手順（`make setup-repository`）として渡すものであり、コードと同じ品質基準で扱う。**テンプレートからの生成はツリーしか写さない** —— ブランチ保護も token の権限も複製されないので、設定そのものではなく「設定を適用する手順」が製品に含まれる([0110](0110-security-operations.md) / [0153](0153-ci-configuration.md))
- **作った側が選ぶべき判断を、boilerplate が代わりに下さない。** 外部へ送る値と送り先を作った側が選んでいないなら、既定は送らない。カタログ build が送る使用状況テレメトリ、解析結果の公開データセットへの登録は、この理由で既定 off とする —— それは作った側の名前についての判断であり、技術的な判断ではない
- **外部アカウントを要する層を同梱してよいのは、作った側が受け取っても費用ゼロで壊れない場合に限る。** 公開リポジトリなら無料で、未設定なら黙って何もしない形にする
...
①は **含めるものの取り込み方**、②は **その取り込みを貫く共通の正当化テスト**(設計者が選択主体か)である。①でデファクトに乗るときも、正当化は②「ベンダーを抜いても正当か」で担保する。③は①②を適用する **宛先** を固定する —— 判断は作った側が受け取る snapshot に向けて下し、それを作った経緯に向けては下さない。
...
- ❌ 作った側が選ぶべき判断(外部への送信・外部データセットへの登録)を boilerplate の既定で有効にすること(§3)
...
- [0011-no-docker.md](0011-no-docker.md) — 表示層ロール / thin proxy / boilerplate の性格。§2 の「フレームワーク選択は別既決」の根拠
...
- [0110-security-operations.md](0110-security-operations.md) / [0153-ci-configuration.md](0153-ci-configuration.md) — 作った側が受け取る姿勢(供給網・CI ハードニング)の中身
