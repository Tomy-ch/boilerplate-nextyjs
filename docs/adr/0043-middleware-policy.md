...
- **認証・セッションの具体モデルは fork 先判断**([0070](0070-backend-role-separation.md)。Next.js 公式も「Proxy をセッション管理・認可に使うな」と明示)。boilerplate 本体は `proxy.ts` に特定の認証実装を組み込まない
...
- [0070-backend-role-separation.md](0070-backend-role-separation.md) — thin proxy / 認証は fork 先 / 確定的認可はデータ境界
