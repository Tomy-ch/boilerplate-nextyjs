...
- **UI カタログとして Storybook を 本リポジトリに採用する**。コンポーネントの視覚的仕様・使い方・状態バリエーションのカタログ置き場を Storybook が担う
...
- **画面固有の業務語彙・API・route を story に埋め込まない。** カタログは 本リポジトリを利用する人が参照する中立な面であり、業務文脈を伴う実例は feature 側の story か画面実装に置く
