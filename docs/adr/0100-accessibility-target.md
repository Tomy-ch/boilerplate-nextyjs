...
boilerplate 本体が満たす**最小の a11y 方針**(WCAG レベル・biome a11y ルールの活用・手動チェックのタイミング)を定める。用途依存で厳格化するのはテンプレートから作った側である。

## 決定

### 1. 目標水準 = WCAG 2.x AA

- boilerplate が目指す水準は **WCAG 2.x レベル AA**(業界標準の既定水準)。作った側が要件に応じて AAA へ引き上げるのは妨げない
