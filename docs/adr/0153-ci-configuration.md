...
- ワークフローは関心事ごとに分ける(go 翻案)。本リポの CI job:
  ...
  - **Actions 定義の静的解析**: 上の 2 つが原理的に見られない観点を **zizmor** で検査する(`make actions-zizmor`)。actionlint も `actions-shellcheck` も、shellcheck へ渡す前に `${{ … }}` を行数だけ保つプレースホルダへ潰す。潰した時点で式は「ただの語」になり、**クオートの有無という script injection の本質が解析対象から消える** —— `run: echo ${{ github.event.issue.title }}` は攻撃者が内容を決められる値をシェルの解釈前に展開するが、shellcheck を通す設計では捕まえられない。正しい形は `env:` 経由で環境変数へ渡し `"$VAR"` で参照することであり、検査はその形へ誘導する。走査対象は workflows と composite action の両方。**落とすのは high の所見だけ**とする(所見の総量ではなく、実行前に止めるべきものだけを止める)。ただし `--min-severity` は表示も絞るため、**全所見を出す実行とゲートの実行を分ける** —— 抑止を severity の引き下げで行っている以上(下記 4)、引き下げた所見まで出力から消えると、[0110](0110-security-operations.md) §3.4 が禁じる「黙って素通り」そのものになる。**hook も CI も `--offline` で走らせる** —— オンラインでしか成立しない監査(impostor-commit 等)を CI 側にだけ足すと、上記 4 の hooks mirror CI が崩れて手元で再現できない失敗が PR にだけ現れる。tag 付け替えの検知は下記 3 の `resolve` が fail-closed で担う。抑制は `.github/zizmor.yml` に理由付きで宣言し、**設定は `--config` で明示的に渡す**(自動探索が外れても「所見ゼロ」ではなく「抑制が効かない」形で現れるため、外れたことに気付けない)。ワークフロー・PR コメント・required check は上の 3 つと同じく `actions-lint` に同居させる(問うているのは同じく「これから CI で実行する Actions の定義が正しいか」)
...
- グループ分け(go の trigger 戦略表の翻案): **CI Checks**(全 PR / マージブロック)/ **Security**(週次スケジュール + 差分が届く PR。[0110](0110-security-operations.md) B10)/ **Deployment**(保護ブランチへの push)/ **Documentation**(portal 配信。[0141](0141-portal-operations.md) D2)
...
- **検査ログ**は即 fail させず結果を capture → **PR コメントを upsert**(HTML マーカーで既存コメント検出 → update / create。go `upsert-pr-comment` の翻案。言語非依存でほぼそのまま)→ 最後に fail-closed(`exit 1`)とする。**coverage** は Vitest 実行後に octocov が LCOV を構造化して PR に報告し、その後に gate の失敗を返す。テストの失敗内容も検査ログとして扱い、同じ機構で投稿する —— octocov が報告するのは coverage だけで、どのテストが落ちたかは言わない
...
- **matrix をランタイムの展開には使わない**: `runs-on: ubuntu-latest` 単一、Node バージョンは `mise.toml` を SSOT とし matrix 展開しない(go の単一ランタイム方針の翻案。[0003](0003-version-manager.md))。禁じているのは**版と OS の掛け合わせ**であって、機構そのものではない
- **仕事を割るための matrix は使う**: 費用が対象の件数に比例し、かつ 1 台の並列度を使い切っている検査は、実時間を縮める手段が台数しか残らない。該当するのは story 単位の VRT で、`vrt.yaml` の `vrt-shard` が `playwright --shard=i/N` で割り、`merge-reports` が 1 本のレポートへ束ね直す。**割った台数は課金に線形へ効く**(fork 先が払う)ので、固定費に対して縮み幅が見合う範囲で止める。**required check にするのは束ねる側の単独 job であり、matrix の側ではない** —— matrix は context 名が実行時に枝分かれし、必須 check がその名前を指定できないため(上記 5 の `required-check-lint` が同じ条件を検査する)
