...
- ❌ 外部 LLM API を呼ぶ操作を、確認なしに実行できる許可へ載せること
- ❌ グラフの答えを網羅性の根拠にすること。網羅が要る問いは grep で裏を取る
- ❌ 出力ディレクトリをリポジトリで追跡すること、および lint の走査へ含めること
- ❌ この道具を CI・git hook・build のいずれかのゲートに接続すること。ゲートの権威は既存の検査にある
