...
- ❌ 他リポジトリの issue / PR を、素の URL・`[text](url)`・`owner/repo#N` で指すこと（既定は `redirect.github.com`）
- ❌ 素のリンクを使う判断をエージェントが自分で下すこと。常設の委任があっても同じ
- ❌ 素のリンクを使うとき、参照元 issue のタイトルを相手リポジトリの読めない言語で書くこと
- ❌ クロスリファレンスを残したあとに、本文の編集で撤回したことにすること
