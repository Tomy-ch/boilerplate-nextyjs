...
**この機構は複製された先へは渡らない**ので、上の判断が及ぶのはこのリポジトリの運用だけである。
渡る機構であれば、セッションへ何が貼られるかを知らないまま読み取りを既定にすることになり、同じ
判断は成立しない。
...
**この機構は複製された先へは渡らない。**問いの主語がこのリポジトリの保守者であり、複製した先のスキル群はそちらで判断する。剥がしの対象として登録する（[0152](0152-agents-md-policy.md)）。
