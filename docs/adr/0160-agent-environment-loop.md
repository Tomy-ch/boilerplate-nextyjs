...
**モデルが要るのは 1 つだけ**である —— 「何が難しかったか」を言うこと。その入力は、決定的な集計が**先に絞ったもの**に限る。**絞りが読む費用を成立させ、読みが絞る値打ちを作る。**
...
- **外へ出るのは読んだ結果であって、記録そのものではない。**抜粋を公開の場へ複製しない

**この機構はテンプレートから作った側へ配らない**ので、上の判断が及ぶのはこのリポジトリの運用だけである。
配る機構であれば、作った側が何をセッションへ貼るかを知らないまま読み取りを既定にすることになり、
同じ判断は成立しない。
...
- ❌ **人の確認なしに外向きの操作を行う。** 起票も投稿も、[0154](0154-claude-skills-operations.md) の確認要件に従う
  ...
  - **例外は closed-loop の送出 1 つだけで、これはこの boilerplate だけの規約である。**宛先を
    エージェントが選ばないこと（`.git` の remote が決める）、外へ出るのが打刻から数えた事実だけで
    記録の抜粋を含まないこと（決定 5）、送る窓が閉じた窓に限られることの 3 つが揃うので、確認が
    守っている判断がここには無い。**3 つのいずれかが崩れたら例外も外す。**
...
**この機構は、テンプレートから作った側には配らない。**問いの主語がこのリポジトリの保守者であり、作った側のスキル群は作った側が判断する。剥がしの対象として登録する（[0152](0152-agents-md-policy.md)）。
...
- [0152-agents-md-policy.md](0152-agents-md-policy.md) — 剥がしの対象
