...
- **A2(ADR 0070・実装 ✅)**: Next.js = UI + 薄い BFF / `/api/*` = thin proxy・業務ロジック禁止 / ドメインはバックエンド / 契約 SSOT = backend の `openapi.gen.yaml` / 境界値所有(response 検証はフロントが最後の砦)/ 認証・セッションの具体はテンプレートから作った側の判断。`src/app/api/**/route.ts` の薄い口、`src/adapters/server/http/` の fetch wrapper、`openapi/` の契約取り込み(`make gen-api`)まで実在する。**Tier 3(A 系)はこれで全 ADR 化完了**
