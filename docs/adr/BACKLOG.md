...
**状態を条件に採らない。**「検出件数が 0 になった」「いまの実測が速い」は前提の変化ではないので、
採ると前提が変わっていないのに決定が動く。書くのは前提そのものの変化 —— 道具が機能を備えたら、
その性質が消えたら、である。**条件を書けない「やらない」は、判断ではなく先送りである。**

**決着して変更まで出た issue に `wontfix` を付けない。** 後から見た人が「検討されずに放置された」と
読む。決定の生存を持つのはこの表であって、issue のラベルではない。
