...
- **B10(ADR 0110・実装 ✅)**: [ADR 0110](0110-security-operations.md)(go 0077 多層防御翻案 / Dependabot cooldown・gitleaks・Trivy 二段・Opengrep / **image-scan・cosign・SBOM は本リポ 0011 no-docker で exclusion**)
...
| **W36** | CMP・IAB TCF 相当の同意管理を同梱しない(軽量機構のみ持つ) | [0131](0131-cookie-consent.md) | **本体が対象法域を 1 つに定めるようになったとき**。同意管理は法域とベンダー選定に強く依存し、本体で一律に決めると作った側の要件を狭める。**「GDPR 対応が要ること」は条件にならない** — gate 述語の消費側は変えずに差し替えられる形が既にあり、差し替えるかは用途依存である |
...
| **W40** | OpenSSF Best Practices バッジを取得しない(`CII-Best-Practices` は 0 のまま置く) | [0142](0142-license.md) | **本リポジトリが、テンプレートではなく成果物そのものとして公開運用へ移るとき**。バッジは*このリポジトリの名前*に紐づく登録で、作った側へは引き継がれない。本リポジトリが取っても作った側の姿勢は 1 ミリも変わらない。**「Scorecard の点が上がること」は条件にならない** |
