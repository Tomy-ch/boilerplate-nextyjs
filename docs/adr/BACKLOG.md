...
- **A2(ADR 0070 として策定済み・実装未)**: 2026-07-13 に [ADR 0070](0070-backend-role-separation.md) として成文化(Next.js = UI + 薄い BFF / `/api/*` = thin proxy・業務ロジック禁止 / ドメインはバックエンド / 契約 SSOT = backend `openapi.gen.yaml`(go ADR 0012 消費者側)/ 境界値所有 = go ADR 0015 翻案・response 検証はフロントが最後の砦 / 認証・セッション具体はテンプレートから作った側の判断)。**Tier 3(A 系)はこれで全 ADR 化完了**
...
| **B7** | 0081 | 観測性 / ロギング | ✅ | ✅ | A2, A5, B3 | server-side logging/observability カーネル / 抽象ロガー(ctx-native・trace_id 自動注入)/ OTel vendor-neutral OTLP-only / signal 別 config gating / 公式 semconv のみ。ブラウザ→BFF 中継 seam で Web Vitals と client の未捕捉例外を集約 / **RUM SaaS は作った側の判断(exclusion)** |
...
- **B10 の内訳**: ローカルは `mise.toml` + `make secret-scan` / `make trivy-fs`。**pre-push hook に載せるのは秘密スキャンだけ**で、脆弱性スキャンは意図的に接続していない([0110](0110-security-operations.md) 3.1 / 撤回条件 W1・W2)。CI 側は `codeql` / `gitleaks`(履歴全体の週次を含む)/ `dependency-scan`(Trivy 二段 + `pnpm audit` ゲート)/ `osv-scan` / `sast` / `bearer` / `dast` / `scorecard` と Dependabot cooldown、`SECURITY.md` が揃っている
<!-- boilerplate-only:replace-with -->
<!-- = - **B9(ADR 0153・実装 ⚠️)**: [ADR 0153](0153-ci-configuration.md)(go workflows 翻案 / 1 関心事=1 workflow・SHA ピン・最小 permissions・hooks mirror / job は biome/tsc/next build/vitest/playwright / matrix 非採用)。lint / typecheck / build / test / smoke と生成物 drift・`uses:` の SHA ピン検査・ドキュメント配信の workflow、`upsert-pr-comment` 基盤を実装済み。visual regression(`vrt`)・画面を通した検証(`e2e`)・Core Web Vitals(`lighthouse`)・container image の digest ピン検査(`images-pin`)も追加済み。セキュリティ系 workflow(B10 側)も着地済み。**残る食い違いは required check の一覧で、`purge-verify` が載っていない** —— 0153 §5「CI Checks グループの job は全て必須」に対する差分はこれだけになった(`component-classes` と `shadcn-manifest` は登録済み)。`make required-check-lint` が見ているのは逆向き(登録済みの context が全 PR で報告されること)なので、この抜けは機械では鳴らない -->
<!-- = - **B10(ADR 0110・実装 ✅)**: [ADR 0110](0110-security-operations.md)(go 0077 多層防御翻案 / Dependabot cooldown・gitleaks・Trivy 二段 / **image-scan・cosign・SBOM は本リポ 0011 no-docker で exclusion**) -->
<!-- = - **B10 の内訳**: ローカルは `mise.toml` + `make secret-scan` / `make trivy-fs`。**pre-push hook に載せるのは秘密スキャンだけ**で、脆弱性スキャンは意図的に接続していない([0110](0110-security-operations.md) 3.1 / 撤回条件 W1・W2)。CI 側は `gitleaks`(履歴全体の週次を含む)/ `dependency-scan`(Trivy 二段 + `pnpm audit` ゲート)/ `osv-scan` / `sast` / `bearer` / `dast` / `scorecard` と Dependabot cooldown、`SECURITY.md` が揃っている -->
...
| 枠 ID | ADR # | タイトル | 選定済み | 実装済み | 依存 | 内容要旨 |
| --- | --- | --- | --- | --- | --- | --- |
| **C1** | 0121 | i18n 戦略 | ✅ | ⬜ | A4, A5 | **exclusion**: i18n ライブラリ本体非同梱(作った側の判断)+ 採用時の App Router seam(proxy.ts/`[locale]`) |
...
| **C8** | 0130 | PWA 戦略 | ✅ | ⬜ | C7 | **exclusion**: Web App Manifest / Service Worker / オフライン本体非同梱(作った側の判断)+ 採用時の `manifest.*` seam |
...
- **C1 / C4(exclusion・追認のため実装物を持たない)/ C6(実装 ✅)**: 2026-07-13 に成文化。用途依存の Tier 5 のため多くは exclusion / 作った側の判断 / Next.js 組込み追認。C1=[0121](0121-i18n-strategy.md)(i18n exclusion)/ C2=[0100](0100-accessibility-target.md)(WCAG AA + biome a11y)/ C4=[0102](0102-browser-support.md)(Next.js 既定 browserslist 追認)/ C5=[0045](0045-fonts-and-images.md)(next/font・next/image)/ C6=[0043](0043-middleware-policy.md)(**Next.js 16 = proxy.ts**・thin・認証は作った側)。go はバックエンドで C 系にほぼ対応物がなく(フロント固有)、AGENTS.md にも C 系 `[TODO]` はない(0152 掲載基準 = ブロック項目のみ)ため BACKLOG C 枠のみを根拠に成文化
...
| **W3** | `mise.toml` の全エントリで backend を明示する | [0003](0003-version-manager.md) | (a) mise がレジストリのマッピング固定を宣言の外で保証する機構を持つ / (b) backend 明示が原因で作った側の環境で解決できない事例が出る。**「記述が冗長」は条件にならない** — 一様適用をやめた時点で規約として機能しなくなる |
...
| **W12** | 強調の段を 1 つ(`emphasis`)だけ持ち、`strong` を持たない | [0051](0051-styling-system.md) §5 | 太さの 2 段目が**全ての対応環境で描き分けられる**ようになったとき — 実測(macOS / Chromium)では游ゴシックが 400 とそれ以外しか描き分けず、`system-ui` も 500 と 600 が同じになる。和文を OS 同梱の書体へ委ねている限りこれは変わらないため、**戻す前提は本文へ和文 Web フォントを置くこと**で、同梱サンプルの実測で CSS が 25 KB から 158 KB へ戻る。**「見出しをもっと強く見せたい」ことは条件にならない** — 階層は寸法と位置が担う |
| **W18** | 呼び出し側が自前で組み立てたコードフェンスを検査しない | [0153](0153-ci-configuration.md) 5 | `details-summary` を使わず本文へ自前でフェンスを組み立てるワークフローを置いた時点。現状は投稿ジョブの全件がアクション側の折り畳みを通るため、フェンス長の決定はアクション 1 箇所に閉じている。**その記法を持ち込む PR が検査も併せて持ち込むこと**が条件であり、置かれるまで検査を先に作らない (対象 0 件の検査は、守っている対象が無いまま緑を返し続ける) |
...
| **W19** | Security グループの検査を、差分が届かない PR では降ろす (許可リストを使うのは依存スキャナだけ) | [0110](0110-security-operations.md) 3.3 / [0153](0153-ci-configuration.md) 2 | **週次スケジュールを外したとき、または Security の job を required check に登録したとき。**この絞りが成立している根拠は「降りた分が週次へ回ること」と「降りても PR が止まらないこと」の 2 つだけで、どちらが消えても書き漏らしが恒久の死角へ変わる。**「絞りのせいで検出が 1 週間遅れた」ことは条件にならない** — 遅れは絞りが意図した挙動そのものである |
| **W20** | 検査が緑のとき PR コメントを新規作成しない (既存コメントの更新は続ける) | [0153](0153-ci-configuration.md) 5 | 緑の実行が**その実行にしか無い情報**を本文で運ぶようになったとき — 現状 PASS のコメントが載せているのは判定だけで、それは check の色が既に述べている。計測値を運ぶもの (`bundle-budget` の route ごとの client JS、`lighthouse` の Core Web Vitals) はこの条件に最も近く、**そちらを戻すなら job ごとに戻す** (`status` は呼び出し側が渡すので、機構は既に job 単位で分かれている)。**「緑の PR にコメントが 1 つも付かないのが寂しい」ことは条件にならない** |
| **W21** | `eslint-plugin-security` の推奨プリセットを当てず、0 件を保てる 9 規則だけを有効にする | [0110](0110-security-operations.md) 3 / [0002](0002-formatter-linter.md) | 落とした 5 規則のいずれかが**形ではなく実体を見るようになったとき** — 例えば `detect-unsafe-regex` が星の高さではなく実際の後戻り計算量で判定するようになれば、`/^\d+(\.\d+)?$/` は鳴らなくなり 0 件を保てる。**「SAST の層が薄い」ことは条件にならない** — ReDoS と path traversal は Opengrep が引き続き担う |
| **W22** | SonarQube Cloud を required check に登録しない | [0110](0110-security-operations.md) 3 / [0153](0153-ci-configuration.md) 5 | **原則としてこの決定は動かない**。動かせるのは、この検査が外部アカウントを要さなくなったときだけである。**「このリポジトリでは常に緑だから」は条件にならない** — 登録が壊すのは作った側であり、しかも壊れ方は「必須チェック待ちで全 PR が止まる」という、コードでは直せない形になる <!-- boilerplate-only:line --> |
...
| **W24** | SAST のルールを semgrep レジストリ (`p/*`) から引かない (`opengrep/opengrep-rules` の commit を固定して読む) | [0110](0110-security-operations.md) 3 | (a) レジストリのルールが OSI 承認ライセンスへ戻ったとき / (b) `opengrep/opengrep-rules` が更新を止め、**他の層でも補えない面**が実測で見つかったとき。現状ルール数は 563 から 77 へ減っている。**「ルールが少ない」「上流の更新が鈍い」ことだけでは条件にならない** — 減ること自体は承知のうえで選んでおり、条件は「減った分がどこにも無い」と実測で言えることである |
| **W25** | i18n 本体(ライブラリ・ロケール解決・翻訳キー体系)を同梱しない | [0121](0121-i18n-strategy.md) | **本体が複数ロケールの文面を持つようになったとき** —— 同梱する画面のいずれかが `[locale]` セグメントか `proxy.ts` のロケール解決を実際に通るようになったときである。設置面の無い seam は腐り、採用時に必ず書き直される。**「作った側に多言語の要望があること」は条件にならない** — 用途依存の判断は作った側が持ち、本体が持つのは採用時の座標だけである |
...
| **W28** | 決済 SDK の mount seam を v1 でコードとして置かない | [0076](0076-payment-ui-seam.md) | **決済画面を本体が同梱するようになったとき**。PCI 境界(生カード情報をフロントに持たせない)は採否によらず不変なので、撤回の対象は mount seam の実体化だけである。**「同梱サンプルが決済らしい画面を持っていること」は条件にならない** — そこを通る支払いは擬似的なもので、PSP の SDK を載せる面が無い |
| **W29** | プロダクト分析の発火 IF / no-op sink を v1 で置かない | [0082](0082-client-observability.md) | **本体がタグへ値を渡すようになったとき**。そこで初めて「直書き vs 抽象を通す」が実在する問題になる([0082](0082-client-observability.md) §3)。**「ゲートの先にタグマネージャが在ること」は条件にならない** — 容器を読み込む口を同梱しても、本体には発火する呼び出しが 1 つも無い |
...
| **W36** | CMP・IAB TCF 相当の同意管理を同梱しない(軽量機構のみ持つ) | [0131](0131-cookie-consent.md) | **本体が対象法域を 1 つに定めるようになったとき**。同意管理は法域とベンダー選定に強く依存し、本体で一律に決めると作った側の要件を狭める。**「GDPR 対応が要ること」は条件にならない** — gate 述語の消費側は変えずに差し替えられる形が既にあり、差し替えるかは作った側の判断である |
| **W37** | 観測性 / RUM の vendor SDK(Sentry / Datadog 等)を同梱しない | [0081](0081-observability-logging.md) 6 | **OTLP で表現できない観測面が実測で見つかったとき** — そのベンダーの SDK を通してしか取れない情報が、運用上どうしても要ると言えるとき。エラー通知・アラートは作った側が向け先に選ぶ OTLP 互換バックエンド側で足りる。**「導入が速いこと」は条件にならない** — 速さは vendor 直参照の恒久コストと釣り合わない |
...
| **W40** | OpenSSF Best Practices バッジを取得しない(`CII-Best-Practices` は 0 のまま置く) | [0142](0142-license.md) | **本リポジトリが、テンプレートではなく成果物そのものとして公開運用へ移るとき**。バッジは*このリポジトリの名前*に紐づく登録で、作った側へは引き継がれない。boilerplate が取っても作った側の姿勢は 1 ミリも変わらない。**「Scorecard の点が上がること」は条件にならない** |
...
| **W44** | 差分からパスを見る宣言(`scripts/deferred-checks/recommend.ts` の `CHECKS` と、待たずに回すと決める `scripts/e2e/trigger.ts` / `scripts/lighthouse/trigger.ts`)が実態から剥がれたことを検出するゲートを置かない | [0153](0153-ci-configuration.md) 2 | **宣言が、ADR かフレームワークの規約で位置の決まっていないパスを持つようになったとき。** 現在の全 10 規則は他所が位置を固定している —— story と `mocks/` の置き場は [0027](0027-directory-structure.md)、token の SSOT は [0051](0051-styling-system.md) 1、`.storybook/` の差し替え宣言は [0054](0054-ui-catalog-storybook.md)、`proxy.ts` は [0043](0043-middleware-policy.md)、`fonts.ts` は [0045](0045-fonts-and-images.md)、`e2e/` は Playwright の `testDir`、`layout.tsx` と `next.config.ts` は Next.js 自身。作った側がそこを動かすことは規約を捨てることを意味するので、宣言だけが黙って古くなる形にならない。もう一方は **`deferred-checks` が required check かゲートに変わったとき** —— 現状は死んだ規則の代償がコメント 1 件の欠落だが、ゲートになれば「検査していないのに緑」へ化ける。**「作った側で規則が死んだ事例が出た」「サンプル破棄で対象が減った」ことは条件にならない** —— どちらも状態であり、実測では破棄後も全 10 規則が対象を持つ。**待たずに回す側(`trigger.ts`)は代償が 1 段重い** —— 死んだ規則の代償はコメントの欠落ではなく、検査が回らないまま merge されることである。それでも同じ条件で足りるのは、挙げているパスの位置を Next.js と Playwright が固定しているためで、作った側がそこを動かすなら規約ごと捨てることになる |
| **W45** | 観測ツールを MCP サーバとしてエージェントへ登録しない (CLI から呼ぶ) | [0156](0156-browser-observation-tooling.md) | **CLI に生成されない道具でしか答えられない問いが実測で現れたとき** — 待機と一括入力は CLI へ生成されないが、待機が要る観測は別レーンが担うため現状は成立している。登録する場合も置き場は利用者のホーム側で、リポジトリには残さない。**「MCP のほうが呼びやすい」ことは条件にならない** — 文脈の常時消費と、作った側へエージェント構成が降ることが対価である |
...
| **W48** | 軽い画面を TBT で締めた行を `performance-budget.yaml` の `screens` に置かない(締める側は LCP だけで書く) | [0101](0101-performance-budget.md) 3 | **割った台ごとに測っている床(`floor`)が、報告だけでなく判定の補正に使われるようになったとき。** TBT は機械の速さがそのまま乗数として効き、器しか持たない画面ほどそれを素直に写すため、いま細い線を引くと遅い実行のたびに床が鳴り、床を見て判別するという役目そのものを失う。補正が入れば、締めた線は機械の速さから切り離される。**「軽い画面の退行を捕まえたい」ことは条件にならない** —— その目的は LCP を締める行が既に果たしており、TBT で二重に引く必要は無い |
| **W49** | 弧の反字体の揺れ(実測 ±3)を story の側で潰さない | [0091](0091-test-verification-methods.md) | **同じ入力の 2 回の撮影がバイトまで一致するようになったとき** —— ラスタライズが並列度に依存しなくなるか、比較が画素の色差ではなく形で判定できるようになったとき。実測では滑走子の円弧やバッジの丸みが 15 × 8 画素以内・最大の差 2 で動き、比較の下限(`threshold: 0.02`)の内側に収まる。**潰す先は弧そのもの**なので、story 側に打つ手が無いことがやらない理由である。守りは撮る側にあり、撮り直しを報告された集合に限ることで揺れが基準へ焼き付くのを止めている。**「撮り直しの差分に説明の付かない行が出た」ことは条件にならない** —— それは全数の撮り直しが起きたということで、そちらの経路を塞ぐ話である |
| **W50** | `AppShell` の header の高さを測らず、定数として押し付ける | [0026](0026-layout-shell-mount.md) | **header の高さが中身で変わる形になったとき** —— 導線が折り返す、行が可変になる等で「決められる量」でなくなったとき。それまでは押し付けている限り書き写した値と描画がずれず、測る形は SSR と初回描画のあいだ 0 を配るため、知り得る量を知るまで間違っている仕組みに置き換えることになる。**「同じ画面で帯の高さは測っているのに不揃いだ」ことは条件にならない** —— 帯は中身の量で折り返すので測る以外に知る方法が無く、種類の違う量である |
| **W51** | 版の宣言の同期検査(`sync-versions-check`)を持たない | [0003](0003-version-manager.md) / [0011](0011-no-docker.md) | **版の宣言が `mise.toml` の外にもう 1 箇所現れたとき。** 上流(go-boilerplate)がこれを持つのは Dockerfile とその README が `mise.toml` の pin を書き写しているためで、本リポジトリは Docker を持たず宣言が 1 箇所しか無い。workflow が版を名乗る唯一の面は `make actions-mise-pin-lint` が既に見ている。**「上流が持っているから」は条件にならない** —— 同期の検査は、同期すべき 2 つ目が在って初めて意味を持つ |
| **W52** | 生成物を bot が commit する仕組み(`auto-generate-docs`)を持たない。生成物の陳腐化は drift 検査で赤くする | [0072](0072-api-type-generation.md) / [0141](0141-portal-operations.md) | **生成物が、人の手で追随できない量や頻度になったとき。** 上流のそれは DB を立てて schema から docs を作り、`contents: write` で自動 PR を開く形で、本リポジトリの生成物(OpenAPI 由来の型・token・portal)はいずれも `gen-drift` / `tokens-drift` / `shadcn-drift` が**落とす**側で守っている。書き込む権限を持つ workflow を増やさずに済むこと自体が、この形を選ぶ理由でもある。**「生成物が増えたこと」は条件にならない** —— 増えても赤くして人が回せる限り、drift 検査の側で足りる |
