...
<!-- = - **B10 の内訳**: ローカルは `mise.toml` + `make secret-scan` / `make trivy-fs`。**pre-push hook に載せるのは秘密スキャンだけ**で、脆弱性スキャンは意図的に接続していない([0110](0110-security-operations.md) 3.1 / 撤回条件 W2)。CI 側は `gitleaks`(履歴全体の週次を含む)/ `dependency-scan`(Trivy 二段 + `pnpm audit` ゲート)/ `osv-scan` / `sast` / `bearer` / `dast` / `scorecard` と Dependabot cooldown、`SECURITY.md` が揃っている -->
...
| # | 決定 (現状) | 権威 | 撤回条件 |
| --- | --- | --- | --- |
| **W2** | `make trivy-fs` を exit code で落とさない | [0110](0110-security-operations.md) 3.1 | **状態では動かない。**動くのは前提が変わったときだけで、(a) 脆弱性をその場で当事者が解消できる機構が入るか、(b) 変更と独立に結果が変動する性質が消えるか。昇格ゲート (保護ブランチ宛 PR) 側は本条件の対象外で、そちらは最初からブロックする |
