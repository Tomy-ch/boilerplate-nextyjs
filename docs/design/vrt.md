...
ruleset が PR を要求するブランチ（`release/**` / `hotfix/**` と各環境のブランチ）へは、gitlink だけを
載せた PR を開く。**その PR は対象ごとに 1 本で、撮り直すたびに開き直さず更新する** —— 撮り直しは
繰り返し起きるので、1 回ごとに 1 本増やすと、マージしなかった提案が溜まっていく側へ倒れる。撮り直しが最も要るのは release ブランチであり、そこだけ経路が無いと機構全体が
そのブランチで成立しない。**保護を緩めて bot に直接 push させる形は採らない** —— 迂回できる保護は
保護ではなく、緩める先は ref 単位に絞れないので `production` まで一緒に開くことになる。経路の判定は
ruleset そのものに問い合わせる。パターンの写しを持つと、保護の宣言と撮り直しの前提が別々に動く。
