...
- **翻案メモ**: 本リポジトリの biome は設定 1 枚(`biome.json`)。P3-2 で ESLint を境界検査に導入する際、**同じ二層を最初から適用する**。無効化したルールを理由付きでコメントカタログ化する様式も輸入
...
- **決着**(v1 計画 P5-18): **採用する / 生成 scaffold は持たない / spec を先に書くことは強制しない**。理由は P5-18 が持つ。仕様書は `docs/spec/route/**` の 2 層で確定している
- **`pnpm gen`(P4-6)の生成入力は `architecture.ts` + 層 README の 1 本に据え置き、改修 PR は起票しない。** spec は生成入力ではなく `new-feature` スキル(P7-3)の読み込み入力として扱う
- **輸入資産の扱い**
  - **破棄**: `new-spec` / `new-spec-{domain,usecase}` / `.claude/scaffold-spec/*` —— いずれも生成 scaffold の資産で、生成器を持たない以上その読み手がいない
...
- **状態**: **完了**(PR #143)。着地した形:
  - ツールの形は §0「輸入の原則」の 1 ツール = 1 ディレクトリのとおり。テストは判定側に置く
  - ゲートの走査対象は `scripts/` に限らず `src/` / `docs-viewer/` / `tokens/` / `mocks/` を含む
...
- **完了条件**: 恒久文書へ前提付きの記述を書くと `pnpm lint:md` が落ちる。許容は理由付きで宣言される
- **依存**: IM-53
...
- go 側が再度強化された場合は §0 の対象スナップショットと §1.1 を取り直し、差分を作業一覧へ追加する。スナップショットは go 側の**現行 release ラインの先頭**で取る(`develop` を基点にしない理由は [IM-59](#im-59-base-branch) の翻案メモ)
- go 側の issue / PR はコードスパンで書く(例: `go-boilerplate#1032`)。理由は [AGENTS.md](../../AGENTS.md) の Cross-Repository Links が持つ
