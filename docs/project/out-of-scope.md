...
- **資格情報の検証と IdP の選定** —— 資格情報は検証せずに中継し、所有するのは認証画面の意匠だけ。
  セッション方式も IdP も作った側の判断（[0079](../adr/0079-auth-frontend-seam.md) / [0070](../adr/0070-backend-role-separation.md)）
...
- **OpenSSF Best Practices バッジ** —— このリポジトリの名前に紐づく登録で、作った側へ引き継がれない（[0142](../adr/0142-license.md)）
...
本体は採用時の座標だけを持つ。作った側が導入する分に、これらの除外は障害にならない。
...
- **作った側のアプリケーションのライセンス** —— MIT は再ライセンスを許す。boilerplate 由来部分の
