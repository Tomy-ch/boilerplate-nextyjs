...
- **その文書より先に失効する前提を、残る文書に書かない。** テンプレートから作る前の状態（同梱サンプルが在ること・整備が途中であること・いま何世代目か）は、作った側では初日に偽になる。前提を書いてよいのは**前提と一緒に捨てられる文書**だけで、それは `README.md` と [`get-started/`](get-started/) である。残る側（ADR / 設計解説 / 仕様書 / 実装規約 / プロジェクトの器 / 層 README）へ書くと、**前提が偽になった瞬間に文書が自分を偽る。** `premise-lint` が残る領域の散文から前提の語を拾う。**判断の要る言い回しは拾わない**ので、拾えない形は同検査が毎回自分で述べる。
...
- **直したものの一般形を、サンプル破棄後も残る側へ回収する。** 直した事実がサンプルのコードにしか無いと、破棄したときに結論ごと消え、テンプレートから作った側が同じ穴を踏み直す。掃くのは **CI が緑になった後・merge の前**である —— 修正は実装中・レビュー・CI の 3 つの時点から出るので、PR を出す前に掃くと最後の 1 つを取り逃がす。列挙は `git log --oneline <base>..HEAD` から機械的に取り、記憶に頼らない。**2 つを落とす** —— 正しい手順を踏めば再発しないエッジケースと、既存のゲートか既存の記述が既に防いでいるもの。「明記されていたのに自分が踏んだ」は文書の穴ではなく読み落としであり、文を足しても直らない。落としたものは報告に明示する。
