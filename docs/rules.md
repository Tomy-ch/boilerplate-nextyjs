...
- **索引させてよい環境（通常は `prd`）だけが `SITE_INDEXABLE=on` を宣言し、それ以外（preview / staging）は既定の `noindex` に留める。** 環境識別バナーの有無は fork 先の要件として Config で決める。
...
- **fork 先で偽になる固有名（特定の認証基盤・環境ごとの案内・実在しないリポジトリ名）を、残る側の文面に書かない。** どの接続先でも真である範囲だけを書く。
