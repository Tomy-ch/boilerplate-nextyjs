...
- **その文書より先に失効する前提を、残る文書に書かない。** テンプレートから作る前の状態（同梱サンプルが在ること・整備が途中であること・いま何世代目か）は、複製した先では初日に偽になる。前提を書いてよいのは**前提と一緒に捨てられる文書**だけで、それは `README.md` と [`get-started/`](get-started/) である。残る側（ADR / 設計解説 / 仕様書 / 実装規約 / プロジェクトの器 / 層 README）へ書くと、**前提が偽になった瞬間に文書が自分を偽る。** `premise-lint` が残る領域の散文から前提の語を拾う。**判断の要る言い回しは拾わない**ので、拾えない形は同検査が毎回自分で述べる。
