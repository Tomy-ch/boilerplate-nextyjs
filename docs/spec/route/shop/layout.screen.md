...
**出し分けは幅の購読で行い、CSS で両方描かない。** これは「出し分けは CSS で行う」
（[`rules.md`](../../../rules.md) の「レイアウトと帯」）の唯一の例外に当たる —— drawer は focus を
閉じ込めるため、CSS で隠しても DOM が残ると広い幅で focus が閉じ込められる。その代わり**初回描画は常に常設側の姿**になり、押せる
