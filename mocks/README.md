...
ハンドラを足せば作れますが、それは「契約が変わればモックも変わる」という一方向を破る唯一の
例外を、この場所に作ることになります。
...
張り直しを繰り返します。したがって発券の取得口が、mock の配備では発券そのものを断ります
（`src/adapters/server/api/inquiries-stream.ts`）。イベントを起こす手段は backend 側が持ちます。
