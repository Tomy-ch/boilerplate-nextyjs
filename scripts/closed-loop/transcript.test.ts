...
import { countTranscript, countUnparsable, neverInvoked } from "./transcript";
...
describe("countTranscript", () => {
  ...
  it("人とモデルの発話をやり取りとして数える", () => {
    const counts = countTranscript([line({ type: "user" }), line({ type: "assistant" }), line({ type: "summary" })]);
  ...
  it("Skill 道具の起動を、スキル名で数える", () => {
    expect(countTranscript([toolUse("Skill", { skill: "commit" })]).skills).toEqual({ commit: 1 });
  });
  ...
  it("`/name` を打った記録も同じスキルの起動として数える", () => {
    ...
    expect(counts.skills).toEqual({ commit: 2 });
  });

  it("ツール自身の組み込みはスキルとして数えない", () => {
    const counts = countTranscript([
  ...
  it("解釈できない形の行を飛ばす", () => {
    const counts = countTranscript([
      ...
      line({ type: "assistant", message: { content: [{ type: "tool_use", name: "Skill", input: null }] } }),
    ]);
