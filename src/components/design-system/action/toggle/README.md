...
focus 表示は `outline` の idiom（`focus-visible:outline-2` / `outline-offset-2` / `outline-active`）に揃えています。押下中と hover の面はどちらも `accent` を使います。生成物は hover に `muted` と `muted-foreground` を当てますが、押下中と同じ `accent` に揃えています。
