 ...
 * なっても印を残す（理由は `stepState`）。
 *
 * **今の段階を終えられないあいだは、先へは進捗からも行けない。** 「次へ」だけを止めても、
 * 進捗から飛べては同じことになる。前へ戻る側は止めない。
 *
...
export function WizardForm({
   ...
   * その段へ移れるか。
   *
   * 到達済みで、いま居る段ではなく、いま居る段を終えられているなら移れる。到達していない段へ
   * 飛べると、呼び出し元が `blocked` で持つ判定を迂回できてしまう。前へ戻る側は
 ...
 * 段階 1 つぶんの領域。
 *
 * 現在の段階以外も DOM へ残すため、`hidden` で隠す。unmount すると `<form action>` で送信した
 * ときにその段の入力値が送られない。`hidden` なら値は form に残ったまま、支援技術と layout からは
 * 外れる。値を保つのが目的なので、`display: none` を class で当てるのではなく属性で隠す。
 *
