...
const meta = {
  ...
  parameters: {
    ...
    docs: {
      ...
      description: {
        component: [
          "管理側の商品一覧です。**利用者側の一覧と同じ商品を、違う形で出します** —— 買う側は 1 件ずつ",
          "眺めて選ぶのでカード、管理側は同じ属性を件どうしで見比べるので表です。行全体が編集への導線",
          "ですが、支援技術には商品名だけが遷移先として見えます。**在庫の数は補充への導線**で、行を押す・",
          "在庫を押すのどちらも、明示的に選ぶ道を行の操作 menu が残します。状態の色は状態名に重ねている",
          "だけで、色だけで区別させません。",
        ].join(""),
      },
    },
