...
const meta = {
  ...
  parameters: {
    ...
    docs: {
      description: {
        component: [
          "期間内の購入をステータスごとに見せる節です。**棒と表を併置します** —— 棒は大小を掴むため、",
          "数そのものは表が持ちます。**合計は出しません**（ここの件数はキャンセルを含み、数値カードの",
          "「売上の件数」は含まないため、足した数はどちらの母集団にも属しません）。並べ替えもしません",
          "—— 契約が返すマスタの表示順のままなので、期間を切り替えても行の位置が動きません。",
        ].join(""),
      },
    },
  },
