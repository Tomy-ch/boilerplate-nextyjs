...
const meta = {
  ...
  parameters: {
    ...
    docs: {
      description: {
        component: [
          "明細を並べ、取り除いた行があった場所に取り消しを差し込む器です。**消えた行と同じ場所に案内を",
          "置きます** —— ゴミ箱を押した場所と案内の出る場所がずれると、どの行が消えたのかを目で辿り直す",
          "ことになります。**場所は番号ではなく並びで持ちます** —— 番号はほかの行が増減するたびに指す先が",
          "変わり、続けて取り除くとずれます。**canvas では取り除けません**（送信先が無いため、案内の出る",
          "姿までは進みません）。",
        ].join(""),
      },
    },
