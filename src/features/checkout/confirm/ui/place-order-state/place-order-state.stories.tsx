...
const meta = {
  ...
  parameters: {
    ...
    docs: {
      description: {
        component: [
          "確定の送信状態を、画面に 1 つだけ置く器です。**同じ集計を 2 か所に描く**ため（脇に貼り付く姿と",
          "下端に固定する帯）、器の出し分けは CSS で行い、DOM には両方が残ります。それぞれが自分の送信",
          "状態を持つと、狭い幅で送って待っている最中に境界を跨いだとき、表に出る側は何も送っていない姿に",
          "なり、**送信中の表示も失敗の文言も消えます**。待っているかを `useFormStatus` ではなく",
          "`useActionState` から採るのはこのためです。",
        ].join(""),
      },
    },
  },
