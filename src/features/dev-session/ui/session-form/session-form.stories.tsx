...
const meta = {
  ...
  parameters: {
    ...
    docs: {
      ...
      description: {
        component: [
          "IdP を通さずに session を発行する指定です。**「API 接続モード」が入っていると、",
          "トークンはこちらで取ります**（実物の API へ繋いでいる間、検証されない前提のトークンは 401 で弾かれるため）。",
          "**貼る欄は、入っていないときだけ出ます。** 認可の往復の途中では素の form 送信になり、送信中の表示と項目ごとの理由はそのとき出ません。",
        ].join(""),
      },
    },
  },
